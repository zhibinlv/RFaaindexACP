...
def fasta2csv(inFasta):
    ...
    for i in range(seqNum):
        ...
        label=name.split("|")[-1]
        csvFile.write(name+","+label+","+str(FastaRead.iloc[2*i+1,0].upper())+"\n")
...
def toFeatureAAindex(infasta,outcsv):

    seqlist=fasta2csv(infasta)
    aaindex_feature=pd.DataFrame()

    for seq in seqlist["Seq"]:
        seqaaidx=seq2AAIndexMat(seq)
        
        aaindex_feature=aaindex_feature.append(seqaaidx, ignore_index =True)
    aaindex_feature=pd.concat([seqlist["Seq"],aaindex_feature],axis=1)
     
         
    aaindex_feature.to_csv(outcsv)
...
if __name__=="__main__":
    ...
    parser = argparse.ArgumentParser(description='Input a sequence file in FASTA')
     
    parser.add_argument('-i', type=str, help='sequence file in fasta')
    parser.add_argument('-o', type=str, help='output AAindex feature file in CSV')
    ...
    toFeatureAAindex(args.i,args.o)
